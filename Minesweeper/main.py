...
class minesweeper():
    ...
    def buildWindow(self, window):
        ...
        self.divLabel = tk.Label(master = statusFrame, text = "Bombs Remaining: {}".format(self.bombsRemaining))
        self.divLabel.grid(row = 1, column = 0)
    ...
    def flag(self, event, row, col):
        ...
        if self.game == True:
            ...
            if not thisCell.isFlagged and not thisCell.isClicked:
                ...
                self.divLabel.config(text="Bombs Remaining {}".format(self.bombsRemaining))

            # If the cell is already flagged...
            elif thisCell.isFlagged and not thisCell.isClicked:
        ...
        if self.game == True:
            ...
            # If the clicked cell has bombs adjacent to it...
                ...
                if self.cellArray[row][column].numAdjacentBombs == -1:
                    ...
                    self.resetButton.config(text="YOU LOSE >:(")

                    
                    for i in range(self.rows):
                        for j in range(self.cols):
                            button = self.buttonArray[i][j]
                            button.config(relief = tk.SUNKEN)
                            if self.cellArray[i][j].numAdjacentBombs != 0:
                                button.config(text=self.cellArray[i][j].numAdjacentBombs)
                                if self.cellArray[i][j].numAdjacentBombs == -1:
    ...
    def reset(self):
        ...
        self.resetButton.config(text="Reset")
